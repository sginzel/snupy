# == Description
# Performs annotation using Omim

class OmimAnnotation < Annotation
	...
	def perform_annotation(file_path, input_vcf)
		...
		begin
			...
			log_file       = File.join(Rails.root, "log", "omim_annotation.log")
			error_log_file = File.join(Rails.root, "log", "omim_annotation.error.log")
	...
	register_tool name:               :omim,
				  label:              "omim",
				  input:              nil, # can be vcf or csv
				  output:             nil, # can be anything really, it is mostly used if your tool returns different outpuf formats
				  supports:           [:none],
	...
	def self.run(cmd, out = OmimAnnotation.logger.instance_variable_get("@logdev").dev, err = OmimAnnotation.logger.instance_variable_get("@logdev").dev)
	cmds   = ["set -o pipefail",
			  "PATH=$PATH:#{OmimAnnotation.bindir}",
			  cmd]
	...
	def self.get_executable(cmd)
		tbxpath = `PATH=$PATH:#{OmimAnnotation.bindir};which #{cmd}`.to_s.strip
		return tbxpath
	end
	...
	def self.load_config(force = false)
		if @@CKCONFIG.nil? or force then
			yaml = File.join(Aqua.annotationdir ,"omim", "omim_config.yaml")
	...
	def self.get_dir(config_name)
		dir = OmimAnnotation.config(config_name)
