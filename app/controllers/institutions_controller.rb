class InstitutionsController < ApplicationController
  ...
  def create
    @institution = Institution.new(params[:institution])
		...
		respond_to do |format|
      if @institution.save
      	#@institution.users = User.find_all_by_id(params[:users])
      	(params[:user] || []).each do |uid, roles|
      		Affiliation.create_from_params(User.find(uid), {@institution.id => roles})
      	end
  ...
  def update
    @institution = Institution.find(params[:id])
		#@institution.users = User.find_all_by_id(params[:users])
		previous_affiliations = @institution.affiliation_ids
    (params[:user] || []).each do |uid, roles|
  		Affiliation.create_from_params(User.find(uid), {@institution.id => roles})
  	end
